...
function Upload() {
  ...
  const [url, setUrl] = useState(''), [files, setFiles] = useState<FileList | null>(null), [projectName, setProjectName] = useState(''), [description, setDescription] = useState(''), [host, setHost] = useState<hostContainer | null>(null), [containerUrl, setContainerUrl] = useState<string | undefined>(), [showExport, setShowExport] = useState(false)
  ...
  const runContainerCommand = async () => {
    ...
    if (command === 'export' && ['zip', 'json'].includes(args[1])) await host.exportFile(args[0], args[1] as 'zip' | 'json')
