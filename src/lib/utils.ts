...
export const executeCommand = (input: string, host: hostContainer | null, projectName: string, setTerminalHistory: any, terminalHistoryRef: any, setTerminalInput: any) => {
  if (!host) return
  useLogStore.setState({ logs: [] });
  const trimmedCommand = input.trim()
  if (!trimmedCommand) return

  setTerminalHistory((prev: any) => [...prev, `${projectName} $ ${trimmedCommand}`])
